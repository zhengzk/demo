...
stc.clazz('stc.component.ImgScroll',function(_cfg) {
    ...
    prev: function() { //上一张
        ...
        $(own.prev_img).css({marginLeft: -_w + 'px',opacity:_opacity.min});
		$(own.img_layer).prepend(own.prev_img);
		$(own.curr_img).animate({opacity:_opacity.min},time);
        $(own.prev_img).animate({marginLeft: 0 + 'px',opacity:_opacity.max},time,function() {
            $(own.curr_img).detach();  //用remove 绑定事件就没了
            own.curr_img = own.prev_img;
			own.direction = 'left';
        });
    },
    next:function() { //下一张
		...
		var _opacity = own.cfg.opacity;
        $(_next).css({marginLeft: 0 + 'px'}).css({opacity:_opacity.min});
		$(own.img_layer).append(_next);
        var _w = $(own.curr_img).width();
		$(_next).animate({opacity:_opacity.max},time);
        $(own.curr_img).animate({marginLeft: -_w + 'px',opacity: _opacity.min},time,function() {
            $(own.curr_img).detach();  //用remove 绑定事件就没了
            own.curr_img = _next;
			own.direction = 'right';
        });
